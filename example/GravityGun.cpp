...
void GravityGun::Update() {
	...
	if (zephyr::ZephyrEngine::Instance().Input().KeyPressed(m_PickUpKey)) {
		// Pick up rigidbody
	...
	} else if (m_Target != nullptr && zephyr::ZephyrEngine::Instance().Input().KeyHold(m_PickUpKey)) {
		// Move picked body
	...
	} else if (m_Target != nullptr && zephyr::ZephyrEngine::Instance().Input().KeyReleased(m_PickUpKey)) {
		// Let go
		...
		m_Target = nullptr;
	} else if (zephyr::ZephyrEngine::Instance().Input().KeyPressed(m_PushKey)) {
		// Push
