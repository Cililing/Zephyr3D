...
void TextRenderer::Draw() const {
    ...
    pos.x = pos.x + m_Offset.x * Engine::Instance().GetWindow().Width();
    pos.y = pos.y + m_Offset.y * Engine::Instance().GetWindow().Height();
    IGUIWidget::Align(&pos.x, -margin.x / 2, Engine::Instance().GetWindow().Width() - margin.x / 2 - text_size.x, m_Horizontal);
    IGUIWidget::Align(&pos.y, -margin.y / 2, Engine::Instance().GetWindow().Height() - margin.y / 2 - text_size.y, m_Vertical);
