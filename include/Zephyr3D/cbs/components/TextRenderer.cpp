...
void zephyr::cbs::TextRenderer::Draw() const {
    ...
    pos.x = pos.x + m_Offset.x * zephyr::ZephyrEngine::Instance().Window().Width();
    pos.y = pos.y + m_Offset.y * zephyr::ZephyrEngine::Instance().Window().Height();
    IGUIWidget::Align(&pos.x, -margin.x / 2, zephyr::ZephyrEngine::Instance().Window().Width() - margin.x / 2 - text_size.x, m_Horizontal);
    IGUIWidget::Align(&pos.y, -margin.y / 2, zephyr::ZephyrEngine::Instance().Window().Height() - margin.y / 2 - text_size.y, m_Vertical);
