...
void zephyr::cbs::ThirdPersonController::Update() {
    // Camera input
    if (ZephyrEngine::Instance().Input().KeyHold(GLFW_MOUSE_BUTTON_2)) {
        // Rotate camera with mouse
        m_XRotation -= glm::radians(ZephyrEngine::Instance().Input().MouseOffset().y * m_MouseSensitivity);
        m_YRotation += glm::radians(ZephyrEngine::Instance().Input().MouseOffset().x * m_MouseSensitivity);
    } else {
        // Rotate camera with numeric keypad
        if (ZephyrEngine::Instance().Input().KeyPressed(GLFW_KEY_KP_1)) {
            // Set camera to look at front
            m_XRotation = 0.0f;
            m_YRotation = 0.0f;
        } else if (ZephyrEngine::Instance().Input().KeyPressed(GLFW_KEY_KP_3)) {
            // Set camera to look at right
            m_XRotation = 0.0f;
            m_YRotation = glm::radians(90.0f);
        } else if (ZephyrEngine::Instance().Input().KeyPressed(GLFW_KEY_KP_7)) {
            // Set camera to look at top
            m_XRotation = ROTATION_LIMIT;
            m_YRotation = 0.0f;
        } else {
            // Rotate
            if (ZephyrEngine::Instance().Input().KeyHold(GLFW_KEY_KP_2)) {
                m_XRotation -= glm::radians(5.0f);
            } else if (ZephyrEngine::Instance().Input().KeyHold(GLFW_KEY_KP_8)) {
                m_XRotation += glm::radians(5.0f);
            }
            if (ZephyrEngine::Instance().Input().KeyHold(GLFW_KEY_KP_4)) {
                m_YRotation -= glm::radians(5.0f);
            } else if (ZephyrEngine::Instance().Input().KeyHold(GLFW_KEY_KP_6)) {
                m_YRotation += glm::radians(5.0f);
            }
        }
    }
